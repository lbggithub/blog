<template>
	<el-select :modelValue="props.labels" @update:modelValue="updateValue" multiple filterable allow-create default-first-option :reserve-keyword="false" placeholder="选择或输入标签">
...
<script setup>
	...
	let optionsLables = ref([]) // 标签库
	// 	获取库中的标签
	call('getLabels').then(res => {
		optionsLables.value = res.data.map(i => {
			return i.name
		})
	})
</script>
